# filename: wb5_roi_los_saddle_refined.py
import numpy as np
import pandas as pd
...
# 三个 ROI 的中心（单位 MW）：
ROI_LOS1_CENTER   = (190.0, 240.0)   # 左侧 LOS
ROI_LOS2_CENTER   = (567.0, 169.0)   # 右侧 LOS2
ROI_SADDLE_CENTER = (567.94, 159.3)   # 右侧 Saddle
...
def main():
    ...
    saddle= solve_opf_zoom(ROI_SADDLE_CENTER, ROI_HALF_SADDLE, sense='max')

    print("[LOS1]", los1['best'])
    print("[LOS2]", los2['best'])
    print("[Saddle]", saddle['best'])
    ...
    if los1['best'] is not None:
        plt.scatter([los1['best']['PG1']], [los1['best']['PG2']],
                    s=150, marker='*', c='crimson', edgecolors='k', linewidths=0.6,
                    label='LOS')
    if los2['best'] is not None:
        plt.scatter([los2['best']['PG1']], [los2['best']['PG2']],
                    s=150, marker='*', c='crimson', edgecolors='k', linewidths=0.6,
                    label='LOS ' if los1['best'] is None else None)
    if saddle['best'] is not None:
        plt.scatter([saddle['best']['PG1']], [saddle['best']['PG2']],
                    s=170, marker='*', c='limegreen', edgecolors='k', linewidths=0.6,
                    label='Saddle Point')
    plt.xlabel("PG1 (MW)  [Generator at bus 1]")
    plt.ylabel("PG2 (MW)  [Generator at bus 5]")
