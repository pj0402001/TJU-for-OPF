    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                grad = p.grad
                if grad.is_sparse:
                    raise RuntimeError("TJU_v4 does not support sparse gradients")
...
# 数据库设置
# ===========================
DB_FILE = "case9_feasible_region.db"
...
def init_db():
    ...
    c.execute("""CREATE TABLE grid_points (
        ...
        P1_MW REAL, loss REAL, steps INTEGER
    )""")
    c.execute("""CREATE TABLE feasible_points (
        ...
        theta1 REAL, theta2 REAL, theta3 REAL, theta4 REAL, theta5 REAL, theta6 REAL, theta7 REAL, theta8 REAL, theta9 REAL,
...
def solve_with_tju(p2_MW, p3_MW,
    ...
    if best_state is None:
        return False, None, best_val, best_step
    ...
    if feasible:
        # 构建完整的电压和相角向量
        Va_full = torch.cat([torch.zeros(1, dtype=dtype, device=device), Va_b])
        Vm_full = torch.cat([torch.ones(3, dtype=dtype, device=device), Vm_b])
        
        return True, {
            'P1': Pg1_b.item() * baseMVA,
            'P2': p2_MW,
            'P3': p3_MW,
            'V': Vm_full.cpu().numpy(),
            'theta': Va_full.cpu().numpy(),
            'Q1': Qg1_b.item() * baseMVA,
            'Q2': Qg2_b.item() * baseMVA,
            'Q3': Qg3_b.item() * baseMVA
        }, best_val, best_step
    else:
        return False, None, best_val, best_step
...
# 固定网格扫描（带进度条与打印）
# ===========================
def scan_fixed_grid_and_visualize(n_points=50):
    """
    ...
    for i, p2 in enumerate(p2_values):
        for j, p3 in enumerate(p3_values):
            t0 = time.time()
            feasible, result, loss, steps = solve_with_tju(
                p2, p3,
                max_steps=3000, tol=5e-6,
                penalty_scale=10.0, penalty_scale_line=20.0,
                lr=0.01, weight_decay=1e-4, use_cosine=True
            ...
            if feasible:
                feasible_count += 1
                # 存储可行点
                c.execute("INSERT INTO feasible_points(P1_MW,P2_MW,P3_MW,V1,V2,V3,V4,V5,V6,V7,V8,V9,theta1,theta2,theta3,theta4,theta5,theta6,theta7,theta8,theta9,Q1,Q2,Q3) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                          (result['P1'], result['P2'], result['P3'],
                           result['V'][0], result['V'][1], result['V'][2], result['V'][3], result['V'][4], 
                           result['V'][5], result['V'][6], result['V'][7], result['V'][8],
                           result['theta'][0], result['theta'][1], result['theta'][2], result['theta'][3],
                           result['theta'][4], result['theta'][5], result['theta'][6], result['theta'][7],
                           result['theta'][8],
                           result['Q1'], result['Q2'], result['Q3']))
                
                # 存储网格点信息
                c.execute("INSERT INTO grid_points(P2_MW,P3_MW,status,message,elapsed_s,P1_MW,loss,steps) VALUES(?,?,?,?,?,?,?,?)",
                          (p2, p3, "feasible", "", dt, result['P1'], loss, steps))
                print(f"[OK ] P2={p2:.2f} MW, P3={p3:.2f} MW -> P1={result['P1']:.2f} MW, loss={loss:.6f}, time={dt:.2f}s")
            else:
                c.execute("INSERT INTO grid_points(P2_MW,P3_MW,status,message,elapsed_s,P1_MW,loss,steps) VALUES(?,?,?,?,?,?,?,?)",
                          (p2, p3, "infeasible", "", dt, None, loss, steps))
                print(f"[FAIL] P2={p2:.2f} MW, P3={p3:.2f} MW -> infeasible, loss={loss:.6f}, time={dt:.2f}s")
...
def visualize_p1_p2():
    ...
    plt.tight_layout()
    plt.savefig("case9_feasible_region.png", dpi=300)
    plt.show()
