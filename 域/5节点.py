# filename: acopf_wb5_ipopt_multistart_domain.py
import math
import numpy as np
...
def build_feas_model(pg5_MW, pg1_MW, init_type='flat', warm=None):
    ...
    if init_type == 'outer':
        ...
        for i in m.BUS:
            m.Va[i].value = math.radians(outer_Va_deg.get(i, 0.0))
        # Vm 仍 1.0
...
def check_feasible_multistart(pg5_MW, pg1_MW, warm_pools, direction):
    ...
    for init_type in ['flat', 'outer']:
        ...
        ok, warm_new = try_feasible(pg5_MW, pg1_MW, init_type=init_type, warm=warm)
        if ok:
            warm_pools[key] = warm_new  # 更新该池
            return True, init_type, warm_pools
    # 全部失败：将该方向两个池清空，避免“坏热启动”带来负迁移
...
def gen_stage2_pg1(feasible_pg1_stage1):
    ...
    for x in feasible_pg1_stage1:
        ...
        for y in np.arange(lo, hi+1e-9, 1.0):
            # 避免包含原来步长5的点，但即使包含也无碍，检查函数会去重
            cand.add(round(y, 3))
    return np.array(sorted(cand))

def gen_stage3_pg1():
    # 缺失区 PG1∈[150,250] 步长 0.5
...
def scan_one_pg5(pg5_MW, enable_stage3=True):
    ...
    for pg1 in pg1_list_inc:
        ok, _, warm_pools = check_feasible_multistart(pg5_MW, pg1, warm_pools, 'inc')
        if ok:
            feasible_pg1.add(round(pg1, 3))
    # 方向 dec
    for pg1 in pg1_list_dec:
        ok, _, warm_pools = check_feasible_multistart(pg5_MW, pg1, warm_pools, 'dec')
        if ok:
            feasible_pg1.add(round(pg1, 3))

    # 阶段二：对阶段一可行点加密 ±20 MW, 步长 1
    if feasible_pg1:
