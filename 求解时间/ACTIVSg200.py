...
# ========== ACOPF 200-bus（ACTIVSg200）分析器 ==========
class ACOPF200Analyzer:
    def __init__(self,
                 case_file='case_ACTIVSg200.m',
                 n_points=15,
                 genA_bus=None,
                 genB_bus=None,
                 range_frac=(0.30, 0.70),
                 solver_opts=None):
        """
        case_file : MATPOWER 案例文件路径（例如 case_ACTIVSg200.m）
        n_points  : 每个维度扫描点数（总求解数为 n_points^2）
        genA_bus, genB_bus : 指定两台扫描机组所在母线号（不指定则自动选择 Pmax 最大的两台在运机组，尽量避开参考母线）
        range_frac: 扫描相对范围（相对于 Pmax），如 (0.30, 0.70) 在 Pmax 的 30%-70% 间扫描
        ...
        # 扫描范围：相对 Pmax，取更易收敛的中间段（例如 30%~70%）
        lo, hi = range_frac
        self.pA_min_pu = max(self.Pmin[self.genA_idx], lo * self.Pmax[self.genA_idx])
        self.pA_max_pu = hi * self.Pmax[self.genA_idx]
        self.pB_min_pu = max(self.Pmin[self.genB_idx], lo * self.Pmax[self.genB_idx])
        self.pB_max_pu = hi * self.Pmax[self.genB_idx]
    ...
    def solve_feasibility(self, pA_pu, pB_pu):
        ...
        try:
            ...
            # 对停机机组，直接 fix 为 0；在运机组按正常上下界
            def pg_bounds(_, g):
                gidx = g - 1
                if self.gen_status[gidx] == 0:
                    return (0.0, 0.0)
                return (self.Pmin[gidx], self.Pmax[gidx])
            def qg_bounds(_, g):
                gidx = g - 1
                if self.gen_status[gidx] == 0:
                    return (0.0, 0.0)
                return (self.Qmin[gidx], self.Qmax[gidx])

            m.Pg = pyo.Var(m.GEN, bounds=pg_bounds)
            m.Qg = pyo.Var(m.GEN, bounds=qg_bounds, initialize=0.0)

            # 显式 fix 停机机组为 0（避免解提取时报未赋值）
            for g in m.GEN:
                gidx = g - 1
                if self.gen_status[gidx] == 0:
                    m.Pg[g].fix(0.0)
            ...
            # 功率平衡（允许一母线多机组，累加该母线所有在运机组 Pg/Qg）
            ...
            # 初值：用 bus 的 VM/VA；有机组母线可用 gen 的 VG 作为电压初值（夹在 Vmin/Vmax 内）
            for i in m.BUS:
                bus_idx = i - 1
                v0 = self.Vm0[bus_idx]
                v0 = min(max(v0, self.Vmin[bus_idx]), self.Vmax[bus_idx])
                m.Vm[i].value = v0
    ...
    def run_scan(self):
        ...
        if self.feasible_times:
            ...
            print("\n未获得可行解。可考虑：扩大扫描范围、增大迭代上限、调整初值/收敛容差。")

# ========== 运行示例 ==========
if __name__ == "__main__":
    # 默认自动选择 Pmax 最大的两台在运机组（尽量不选参考母线），扫描区间为各自 Pmax 的 30%~70%
    # 每维 15 点，共 225 次求解（可按机器性能调整 n_points）
    analyzer = ACOPF200Analyzer(
        case_file='case_ACTIVSg200.m',  # 请确保该文件在同一目录
        n_points=15,
        genA_bus=None,  # 可设置具体母线号，例如 101
        genB_bus=None,  # 可设置具体母线号，例如 152
        range_frac=(0.30, 0.70),
        solver_opts={
            'max_iter': 300,
            'tol': 1e-6,
