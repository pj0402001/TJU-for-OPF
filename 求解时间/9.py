...
class ACOPFAnalyzer:
    def __init__(self, config_file='ac_opf_config.json'):
        ...
        # 新增：存储每个可行解的求解时间（秒）
        self.feasible_times = []
        self.avg100_done = False  # 防止重复计算100个平均
    ...
    def setup_database(self):
        ...
        if os.path.exists(self.db_name) and self.config['database'].get('auto_clear', True):
            try:
                ...
                temp_conn.execute('DROP TABLE IF EXISTS metrics')
                temp_conn.commit()
                temp_conn.close()
                print(f"🗑️  已清除旧数据库数据")
    ...
    def save_avg_time_100(self, avg_time):
        """保存前100个可行解的平均求解时间到数据库"""
        self.conn.execute('''
            INSERT OR REPLACE INTO metrics (id, avg_time_100, computed_at)
            VALUES (1, ?, ?)
        ''', (avg_time, datetime.now().isoformat()))
        self.conn.commit()
        ...
        for i, p2 in enumerate(p2_values):
            for j, p3 in enumerate(p3_values):
                ...
                if is_feasible and solution_data:
                    ...
                    # 记录每个可行解时间
                    self.feasible_times.append(calc_time)
                    # 打印每个可行解的用时（秒），保留3位小数
                    print(f"✅ 可行解: P2={p2:.1f} MW, P3={p3:.1f} MW, 用时={calc_time:.3f}s")

                    # 新增：当累计可行解达到100个时，计算平均求解时间（只计算一次）
                    if (not self.avg100_done) and len(self.feasible_times) >= 100:
                        avg100 = sum(self.feasible_times[:100]) / 100.0
                        self.avg100_done = True
                        self.save_avg_time_100(avg100)
                        print(f"🎯 已获得100个可行解，前100个的平均求解时间 = {avg100:.3f}s（结果已写入数据库 metrics）")

                # 如需打印不可行点的时间，取消下一行注释
                # else:
                #     print(f"❌ 不可行: P2={p2:.1f} MW, P3={p3:.1f} MW, 用时={calc_time:.3f}s")
    ...
    def estimate_time_left(self, completed, total, start_time):
        ...
        remaining_seconds = remaining_points / rate if rate > 0 else float('inf')
