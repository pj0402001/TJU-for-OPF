...
# ========== ACOPF 500-bus（ACTIVSg500）分析器 ==========
class ACOPF500Analyzer:
    def __init__(self,
                 ...
                 solver_opts=None):
        """
        case_file : MATPOWER 案例文件路径（例如 case_ACTIVSg500.m）
        n_points  : 每个维度扫描点数（总求解数为 n_points^2）
        genA_bus, genB_bus : 指定两台扫描机组所在母线号（不指定则自动选择 Pmax 最大的两台在运机组，尽量不同母线且避开参考母线）
        range_frac: 扫描相对范围（相对于 Pmax），如 (0.35, 0.75) 在 Pmax 的 35%-75% 间扫描
        ...
        # 扫描范围：取更易收敛的中间段（默认 35%~75% 的 Pmax）
        lo, hi = range_frac
        self.pA_min_pu = max(self.Pmin[self.genA_idx], lo * self.Pmax[self.genA_idx])
        self.pA_max_pu = min(self.Pmax[self.genA_idx], hi * self.Pmax[self.genA_idx])
        self.pB_min_pu = max(self.Pmin[self.genB_idx], lo * self.Pmax[self.genB_idx])
        self.pB_max_pu = min(self.Pmax[self.genB_idx], hi * self.Pmax[self.genB_idx])
        ...
        # 打印信息与机组容量Top列表
        print("===== ACTIVSg500 节点 AC-OPF 可行性扫描 =====")
        ...
        for gi in top:
            print(f"  #{gi+1:<3} | {int(self.gen_bus[gi]):<4} | {self.Pmax[gi]*self.baseMVA:>8.1f}")
...
if __name__ == "__main__":
    # 默认：自动选择 Pmax 最大的两台在运机组（尽量不选参考母线且不同母线），
    # 扫描区间为各自 Pmax 的 35%~75%，每维 12 点（共 144 次求解）。
    # 500节点系统计算量大，建议先用较小的 n_points 验证流程（如 10），再根据性能调整。
    analyzer = ACOPF500Analyzer(
        case_file='case_ACTIVSg500.m',   # 请确保该文件在同一目录
        n_points=12,                      # 可根据机器性能调整（10~15）
        genA_bus=None,                    # 也可指定具体母线号
        genB_bus=None,                    # 也可指定具体母线号
        range_frac=(0.35, 0.75),
        solver_opts={
            'max_iter': 500,              # 500节点，适当提高迭代上限
            'tol': 1e-6,
            'acceptable_tol': 1e-4,
            'print_level': 0,
            'linear_solver': 'mumps'
